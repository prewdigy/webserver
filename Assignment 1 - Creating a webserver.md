...
Initially I got an error about a Nginx not starting due to a timeout.  (./work project nginx status.png)

I then ran 

`sudo systemctl status nginx` 

to see if there was any better error messages explaining why it had timed out.  There were none so I just tried running the start command again and it worked.![it works](work project nginx status active.png)
...
To verify that all of this was done correctly I tried to access the IP address from my phone and got a "the site can't be reached" page.

But when accessing from my work computer with the VPN connected I was brought to the Nginx default page.  Once I disconnected from the VPN I get a 404 error as well.
